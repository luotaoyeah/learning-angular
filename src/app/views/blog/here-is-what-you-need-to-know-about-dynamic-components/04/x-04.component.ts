...
export class X04Component {
  ...
  public createComponent() {
    const X0401Component = Component({
      template: '<button class="ant-btn ant-btn-default"><span>{{foo}}</span></button>',
    })(
      class {
        public foo: string = '';
      },
    ...
    this.compiler.compileModuleAndAllComponentsAsync(X0401Module).then(({ componentFactories }) => {
      this.vc.clear();
