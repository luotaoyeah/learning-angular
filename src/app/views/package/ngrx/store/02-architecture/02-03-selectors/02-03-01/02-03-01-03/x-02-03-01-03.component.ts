...
export class X02030103Component implements OnInit {
  ...
  constructor(
    ...
    this.counter01$ = store.pipe(
       ...
       */
      select(
        createSelector((state: { '02-03-01-03': IAppState }) => state['02-03-01-03'], selectCounterMultiply01),
        { id: '1', multiply: 2 },
      ),
    );

    const selectCounterMultiply02Factory = () => {
      return createSelector((state: { '02-03-01-03': IAppState }) => {
        return state['02-03-01-03'];
      }, selectCounterMultiply02);
    };
    this.counter02$ = store.pipe(select(selectCounterMultiply02Factory(), { id: '1', multiply: 2 }));
    this.counter03$ = store.pipe(select(selectCounterMultiply02Factory(), { id: '1', multiply: 3 }));
  }
